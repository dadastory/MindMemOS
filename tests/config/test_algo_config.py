...
def test_structured_add_defaults_are_bounded_and_generic() -> None:
    ...
    assert structured.extraction.selection_enabled is True
    assert structured.extraction.coverage_validation_enabled is True
    assert structured.extraction.max_coverage_items == 12
...
def test_structured_search_defaults_to_a_conservative_dense_relevance_floor() -> None:
    structured = build_config(config_path="config/mindmemos/dev.example.yaml").algo_config.search.structured

    assert structured.min_relevance_score == 0.35
