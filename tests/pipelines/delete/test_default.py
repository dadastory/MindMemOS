...
class FakeWriter:
    ...
    async def apply_mutation_plan(self, ctx: MemoryRequestContext, plan) -> MemoryDbWriteResult:
        ...
        for command in plan.memory_deletes:
            self.calls.append((ctx.project_id, command.memory_id, command.consistency, command.hard))
...
async def test_delete_archives_existing_memory() -> None:
    ...
    assert writer.calls == [("proj-1", "mem-1", "strong", False)]
    assert result.status == "ok"
    assert result.message is None


@pytest.mark.asyncio
async def test_delete_forwards_explicit_hard_flag() -> None:
    writer = FakeWriter(changed=True)
    pipeline = DefaultDeletePipeline(db_reader=FakeReader(), db_writer=writer)

    result = await pipeline.delete(DeletePipelineInput(memory_id="mem-1", hard=True), make_context())

    assert writer.calls == [("proj-1", "mem-1", "strong", True)]
    assert result.status == "ok"
