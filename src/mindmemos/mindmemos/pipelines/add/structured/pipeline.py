...
class StructuredAddPipeline(MemoryDbPipelineMixin, AddPipeline):
    ...
    async def _prepare_extraction(
        ...
        for entity in extracted.get("entities", []):
            ...
            entity_texts.setdefault(
                entity_id,
                "\n".join(filter(None, [entity["name"], entity.get("description", ""), entity["entity_type"]])),
            )
        ...
        texts = [item.content for item in properties] + list(entity_texts.values())
        vectors = (
            await batch_embed(
                embed,
                texts,
    ...
    async def _decisions(
        ...
        *,
        ...
        current_episode_id = str(current_episode.get("episode_id") or "")
        ...
        *,
        ...
        if revision_indexes:
            embedded_revisions = await batch_embed(
                embed,
                [decisions[index].merged_content or properties[index].content for index in revision_indexes],
                batch_size=config.embedding.batch_size,
    ...
    async def _build_mutation_plan(
        ...
        *,
        ...
        get_entity = getattr(self.db_reader, "get_entity", None)
        existing_episode = (
            await get_entity(context, selected_episode_id)
            if get_entity is not None and selected_episode_id is not None
            else None
        )
        if (
            episode_decision is not None
            and selected_episode_id is not None
            and episode_decision.action == "create"
            and existing_episode is None
        ):
            episode_write = EntityWrite(
                entity_id=selected_episode_id,
                ...
                request_id=context.request_id,
                entity_name=episode_decision.title,
            ...
            prepared_episode = preprocessor.preprocess_text(
                ...
                segment_id=selected_episode_id,
            ...
            flat.entity_vectors.append(
                EntityVectorWrite(
                    entity_id=selected_episode_id,
                    semantic_vector=episode_vector,
        ...
        if episode_decision is not None and selected_episode_id is not None:
            flat.relationships.extend(
                _related_episode_relationship(context.project_id, selected_episode_id, related_episode_id)
                for related_episode_id in dict.fromkeys(episode_decision.related_episode_ids)
                if related_episode_id != selected_episode_id
            )

        for item, decision in zip(properties, decisions, strict=True):
            if not decision.target_id:
                continue
            selected_target = await self.db_reader.get_memory(context, decision.target_id)
            if selected_target is not None and selected_target.status == "active" and selected_target.entity_id:
                item.entity_id = selected_target.entity_id
        ...
        for item_index, (item, provisional) in enumerate(zip(properties, decisions, strict=True)):
            ...
            exact_same_episode = bool(
                exact is not None
                and (
                    selected_episode_id is None
                    or selected_episode_id in exact.episode_ids
            ...
            if decision.action == "reinforce" and target is not None:
                ...
                if selected_episode_id and target.entity_id:
                    flat.relationships.append(
                        _episode_relationship(context.project_id, target.entity_id, selected_episode_id)
                    )
                events.append(
                    MemoryAddEventItem(
            ...
            if decision.action in {"update", "supersede"} and target is not None:
                ...
                if exact is not None and exact.status == "active" and not exact_same_episode:
                    event_key = selected_episode_id or inp.idempotency_key or add_record_id or context.request_id
            ...
            prepared = preprocessor.preprocess_text(content, segment_id=memory_id, include_entities=False)
            ...
            if selected_episode_id:
                flat.relationships.append(
                    _episode_relationship(context.project_id, item.entity_id, selected_episode_id)
                )
            if target is not None and decision.action in {"update", "supersede"}:
                flat.relationships.extend(
                    _memory_derivation_relationship(
    ...
    async def _prepare_entity_writes(
        ...
        for entity_id, aliases in aliases_by_id.items():
            ...
            entity = EntityWrite(
                ...
                agent_id=context.agent_id,
                request_id=context.request_id,
                entity_name=existing.entity_name if existing is not None else raw["name"],
                entity_type=existing.entity_type if existing is not None else raw["entity_type"],
                description=(existing.description or raw.get("description", ""))
...
def _is_exact_with_entity_context(
    ...
    *,
    ...
    if memory.entity_type != item.entity_type or memory.property_name != item.property_name:
        return False
    if canonical_content(memory.content) != canonical_content(item.content):
        return False
